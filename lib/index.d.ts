...
export type ColumnSortFunc<T, E extends keyof T = any> = (a: T[E], b: T[E], order: "asc" | "desc", dataField: any, rowA: T, rowB: T) => number;
...
export type HeaderSortingClasses<T extends object = any, E = any> =
  | string
  | ((column: ColumnDescription<T, E>, sortOrder: "asc" | "desc", isLastSorting: boolean, colIndex: number) => string);
...
export type ColumnFormatter<R, E = any, C = any> = (cell: C, row: R, rowIndex: number, formatExtraData?: E) => React.ReactNode;
...
export interface ColumnDescription<T extends object = any, E = any> {
  ...
  footer?: boolean | number | string | ((columnData: any, column: ColumnDescription<T, E>, columnIndex: number) => string) | undefined;
  ...
  validator?:
    | ((newValue: any, row: T, column: ColumnDescription<T, E>, done: (result?: ValidationResult) => any) => boolean | ValidationResult)
    | undefined;
...
export interface SelectRowProps<T> {
  ...
  selectionRenderer?: ((options: { checked: boolean; disabled: boolean; mode: string; rowIndex: number; rowKey: string }) => JSX.Element) | undefined;
  ...
  selectColumnStyle?:
    | ((props: { checked: boolean; disabled: boolean; rowIndex: number; rowKey: string }) => CSSProperties | undefined)
    | CSSProperties
    | undefined;
...
export interface BootstrapTableProps<T extends object = any, K = number> {
  ...
  remote?: boolean | Partial<{ pagination: boolean; filter: boolean; sort: boolean; cellEdit: boolean; search: boolean }> | undefined;
